...
import { scales, PolysafemoonToggleProps, HandleProps, InputProps, ScaleKeys } from "./types";
...
const getScale =
  (property: ScaleKeys) =>
  ({ scale = scales.MD }: PolysafemoonToggleProps) => {
...
export const PancakeStack = styled.div<HandleProps>`
  ...
  .pancake {
    ...
    top: 2px;
    left: 4px;
    box-shadow: 0 ${getScale("pancakeThickness")} 0 ${getScale("pancakeThickness")} #9A6AFF;
  }

  .pancake:nth-child(1) {
    background: ${({ theme }) => theme.PloysafemoonToggle.handleBackground};
    box-shadow: 0 ${getScale("pancakeThickness")} 0 ${getScale("pancakeThickness")}
      ${({ theme }) => theme.PloysafemoonToggle.handleShadow};
  }
...
export const PancakeInput = styled.input<InputProps>`
  ...
  &:checked + label .pancake:nth-child(1) {
    background: #5012d1;
    box-shadow: 0 ${getScale("pancakeThickness")} 0 ${getScale("pancakeThickness")} #905efa;
...
export const PancakeLabel = styled.label<PolysafemoonToggleProps>`
